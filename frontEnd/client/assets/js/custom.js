  ...
  if ($("#slider-range").length) {
    $("#slider-range").slider({
      ...
      slide: function (event, ui) {
        ...
        var formattedValue2 = formatCurrency(ui.values[1]);
        $("#amount").val(formattedValue1 + " - " + formattedValue2);
      }
    });

    // Initialize the input with initial slider values
    var initialValue1 = formatCurrency($("#slider-range").slider("values", 0));
    var initialValue2 = formatCurrency($("#slider-range").slider("values", 1));
    $("#amount").val(initialValue1 + " - " + initialValue2);

    // Function to format number with currency symbol and thousands separator
    function formatCurrency(value) {
      return value.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' }).replace(/\s/g, '');
    }
  }
