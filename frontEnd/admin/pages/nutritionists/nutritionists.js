...
async function _init() {
    var items = await Helper.fetchData("nutritional")
    ...
    items.forEach(item => {
        ...
        cloneitemDiv.querySelector("._buttonDelete").href = "../../backend/?controller=nutritional&action=delete&id=" + item.ID
...
async function _sendData() {
    document.querySelector("#exampleModal .modal-footer .btn-primary").addEventListener('click', async () => {
        ...
        const url = "../../backEnd/?controller=nutritional&action=insert";
...
async function _searchData() {
    document.querySelector(".search-area button").addEventListener('click', () => {
        var keyWord = document.querySelector(".search-area input").value.toUpperCase()
        console.log(keyWord);
        document.querySelectorAll("#example5 tbody tr").forEach(item => {
            item.querySelector(".text-nowrap").textContent.toUpperCase().includes(keyWord) ? item.classList.remove("d-none") : item.classList.add("d-none")
            console.log(item.querySelector(".text-nowrap").textContent.toUpperCase());
        })
    })
}